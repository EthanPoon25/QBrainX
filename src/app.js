...
import "./app.css"
import {
  ResponsiveContainer, LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, AreaChart, Area, BarChart, Bar,
} from "recharts"
...
const genRewards = (len = 120, offset = 0) =>
...
const genMolecules = (n = 8) =>
  Array.from({ length: n }, (_, id) => {
    const bits = Array.from({ length: 16 }, () => (Math.random() > 0.52 ? 1 : 0))
...
/* ---------------------------------- Helpers ---------------------------------- */
function hslBlueRed(t) {
  // t in [0,1], map to blue->red gradient
  const hue = 240 - 240 * t
  const sat = 85
  const light = 52 + 18 * t
...
function BrainPanel({ synapses, rewards, params, setParams, metrics }) {
  return (
    <div className="panel">
          ...
          <div className="panel-body" style={{ height: 360 }}>
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={rewards}>
                ...
                <Tooltip formatter={(v, n) => n === "reward" ? `${(v*100).toFixed(1)}%` : v} contentStyle={{ background: "#0b1020", border: "1px solid rgba(158,173,214,0.35)", borderRadius: 10, color: "var(--text)" }}/>
...
function MoleculesPanel({ molecules, onRegenerate, selected, setSelected, onExport, onFileUpload }) {
  return (
    <div className="panel">
      <div className="panel-header">
              ...
              <div className="mol-list">
                {molecules.map((m, idx) => (
                  <div key={m.id} className={`mol-item ${selected === idx ? "active" : ""}`} onClick={() => setSelected(idx)} role="button" aria-label={`Select molecule ${m.structure}`}>
                    <div className="fp-bits" aria-hidden>
              ...
              {molecules[selected] ? (
                <>
                  ...
                  <button className="btn" onClick={() => {
                    ...
                    if (!window.$3Dmol) { alert('3Dmol.js not loaded'); return }
                    const viewer = window.$3Dmol.createViewer(elementId, { backgroundColor: 'white' })
                    viewer.clear()
...
function QuantumPanel({ binding, vqeSteps, setVqeSteps, onRerun }) {
  return (
    <div className="panel">
      <div className="panel-header">
...
function ControlsPanel({ status, setStatus, params, setParams, onStartPause, onReset, onSave, onLoad }) {
...
function LoginSignup({ onLogin, onSignup, isLogin = true }) {
...
function UserHistory({ history, onLoadHistory }) {
  return (
...
function FileUpload({ onFileUpload, onRegenerate }) {
...
export default function App() {
  ...
  // Brain state
  const [synapses, setSynapses] = useState(() => genSynapses())
  const [rewards, setRewards] = useState(() => genRewards())
  const [binding, setBinding] = useState(() => genBinding())
  const [molecules, setMolecules] = useState(() => genMolecules())
  const [selectedMol, setSelectedMol] = useState(0)

  const [vqeSteps, setVqeSteps] = useState(60)
  ...
  const [params, setParams] = useState({
    ...
    lambda: 0.55,
    seed: 42,
    modBrain: true, modVqe: true, modPlas: true, modDisc: true,
  })
  ...
  const handleSignup = (newToken, email) => {
    setToken(newToken)
    setUser({ email })
    setShowLogin(true)
    setUserHistory([])
  ...
  const loadUserHistory = async (userToken) => {
    try {
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      ...
      setRewards(prev => {
        const lastEpoch = prev.length ? prev[prev.length - 1].epoch + 1 : 0
  ...
  }, [status.running, metrics.rewardMean, binding.length])
  ...
  function handleExportMolecules() {
    ...
    <div className="container" role="main">
      ...
      {tab === "brain" && (
        <BrainPanel
          synapses={synapses}
          rewards={rewards}
          params={{ plasticity: params.plasticity, noise: params.noise, enablePlasticity: params.enablePlasticity }}
